...
function pointermove(event: PointerEvent): void {
	...
	const velocityTop = Math.max(0, (containerFrame.top + Y_SENSOR - clientY) / Y_SENSOR);
	const velocityBottom = Math.max(0, (clientY - (containerFrame.bottom - Y_SENSOR)) / Y_SENSOR);
...
function resizePointerMove({ pageX, pageY }: { pageX: number; pageY: number }): void {
...
function pointerdown({ clientX, clientY, target }: { clientX: number; clientY: number; target: unknown }): void {
...
const svltGridShadowTransform = $derived(`translate(${shadow.x * xPerPx + gapX}px, ${shadow.y * yPerPx + gapY}px)`);
</script>

<div
	draggable={false}
	onpointerdown={item && item.customDragger ? null : !draggable ? null : pointerdown}
